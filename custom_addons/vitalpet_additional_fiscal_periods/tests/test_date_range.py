...
class DateRangeTest(TransactionCase):
    ...
    def test_default_company(self):
        account_fiscal_periods = self.env['date.range']
        dt = account_fiscal_periods.create({
            'name': 'FS2016',
    ...
    def test_overlap(self):
        ...
        # check it's possible to overlap if it's allowed by the Account Fiscal Period Type
        self.type.allow_overlap = True
        dr = account_fiscal_periods.create({
            'name': 'FS2016',
            'date_start': '2015-01-01',
            'date_end': '2016-12-31',
            'type_id': self.type.id,
