...
class PromotePost(models.Model):
    ...
    def create(self, vals):
        post = super(PromotePost, self).create(vals)
        ...
        if 'image' in vals and post.image:
            fname = '%s_%s.jpg' % (post.blog_id.name, post.id)
    ...
    @api.multi
    def write(self, vals):
        res = super(PromotePost, self).write(vals)
        s3 = self._get_s3_resource()
        if not s3:
            _logger.info(_('Amazon S3 source not defined properly. Attachments won\'t be saved on S3.'))
            return res
        ...
        bucket_name = self._get_s3_settings('s3.bucket', 'S3_BUCKET')
        for post in self:
            if 'image' in vals or (post.image and not post.image_s3_url):
                fname = '%s_%s.jpg' % (post.blog_id.name, post.id)
                s3.Bucket(bucket_name).put_object(
                    Key=fname,
