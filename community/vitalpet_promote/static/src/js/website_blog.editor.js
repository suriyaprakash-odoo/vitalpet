odoo.define('vitalpet_promote.new_promote_post', function (require) {
"use strict";
...
contentMenu.TopBar.include({
	new_promote_post: function () {
        var model = new Model('promote.blog');
        model.call('name_search', [], { context: base.get_context() }).then(function (blog_ids) {
            if (blog_ids.length === 1) {
                document.location = '/promote/' + blog_ids[0][0] + '/post/new';
            } else if (blog_ids.length > 1) {
                website.prompt({
                    id: "editor_new_blog",
                    window_title: _t("New Promote Post"),
                    select: "Select Blog",
                    init: function (field) {
                        return blog_ids;
                    },
                }).then(function (blog_id) {
                    document.location = '/promote/' + blog_id + '/post/new';
                });
            }
        });
    },
...
odoo.define('vitalpet_promote.editor', function (require) {
    "use strict";
    ...
    if(!$('.vitalpet_promote').length) {
        return $.Deferred().reject("DOM doesn't contain '.vitalpet_promote'");
    }
    ...
    rte.Class.include({
        ...
        saveElement: function ($el, context) {
            if ($el.is('.o_blog_cover_container')) {
                return ajax.jsonRpc("/promote/post_change_background", 'call', {
                    'post_id' : parseInt($el.closest("[name=\"promote_post\"], .vitalpet_promote").find("[data-oe-model=\"promote.post\"]").first().data("oe-id"), 10),
                    'cover_properties' : {
    ...
    options.registry.many2one.include({
        select_record: function (li) {
            ...
            if (this.$target.data('oe-field') === "author_id") {
                var $nodes = $('[data-oe-model="promote.post"][data-oe-id="'+this.$target.data('oe-id')+'"][data-oe-field="author_avatar"]');
                $nodes.each(function () {
