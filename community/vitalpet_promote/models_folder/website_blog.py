...
class Blog(models.Model):
    ...
    def write(self, vals):
        ...
        if 'active' in vals:
            ...
            post_ids = self.env['promote.post'].with_context(active_test=False).search([
                ('blog_id', 'in', self.ids)
            ])
            for promote_post in post_ids:
                promote_post.active = vals['active']
        return res
    ...
    def message_post(self, parent_id=False, subtype=None, **kwargs):
        ...
        if parent_id:
            ...
            if parent_message.subtype_id and parent_message.subtype_id == self.env.ref('vitalpet_promote.mt_blog_blog_published'):
    ...
    def all_tags(self, min_limit=1):
        req = """
            SELECT
                p.blog_id, count(*), r.promote_tag_id
            FROM
                promote_post_promote_tag_rel r
                    join promote_post p on r.promote_post_id=p.id
            WHERE
                p.blog_id in %s
            GROUP BY
                p.blog_id,
...
class BlogTag(models.Model):
    ...
    post_ids = fields.Many2many('promote.post', string='Posts')
...
class BlogPost(models.Model):
    ...
    def _compute_website_url(self):
        ...
        for promote_post in self:
            promote_post.website_url = "/promote/%s/post/%s?enable_editor=1" % (slug(promote_post.blog_id), slug(promote_post))

    @api.multi
    def _compute_ranking(self):
        res = {}
        for promote_post in self:
            if promote_post.id:  # avoid to rank one post not yet saved and so withtout post_date in case of an onchange.
                age = datetime.now() - fields.Datetime.from_string(promote_post.post_date)
                res[promote_post.id] = promote_post.visits * (0.5 + random.random()) / max(3, age.days)
        return res
                ...
                <div class="container">
                    <div class="row">
                        <div class="col-md-12 mb16 mt16">
                            <p class="o_default_snippet_text">''' + _(" ") + '''</p>
                        </div>
                    </div>
                </div>
            </section>
    ...
    name = fields.Char('Title', required=False, translate=True, default='')
    ...
    blog_id = fields.Many2one('promote.blog', 'Blog', required=True, ondelete='cascade')
    tag_ids = fields.Many2many('promote.tag', string='Tags')
    ...
    def on_change_blog_id(self):
        categs = self.env['promote.blog'].search(
            [('name', 'not in', ['Reviews'])])
        return {'domain':{'blog_id':[('id','in',categs.ids)]}}
    
    @api.multi
    def publish_unpublish_button(self):
        self.website_published = not(self.website_published)
        
    @api.multi
    def website_publish_button(self):
        self.ensure_one()
        if self.env.user.has_group('vitalpet_promote.group_promote_blogger') and self.website_url != '#':
    ...
    def _compute_teaser(self):
        for promote_post in self:
            if promote_post.teaser_manual:
                promote_post.teaser = promote_post.teaser_manual
            else:
                content = html2plaintext(promote_post.content).replace('\n', ' ')
                promote_post.teaser = content[:150] + '...'

    @api.multi
    def _set_teaser(self):
        for promote_post in self:
            promote_post.teaser_manual = promote_post.teaser

    @api.multi
    @api.depends('create_date', 'published_date')
    def _compute_post_date(self):
        for promote_post in self:
            if promote_post.published_date:
                promote_post.post_date = promote_post.published_date
            else:
                promote_post.post_date = promote_post.create_date

    @api.multi
    def _set_post_date(self):
        for promote_post in self:
            promote_post.published_date = promote_post.post_date
            if not promote_post.published_date:
                promote_post._write(dict(post_date=promote_post.create_date)) # dont trigger inverse function

    def _check_for_publication(self, vals):
        if vals.get('website_published'):
            for post in self:
                post.blog_id.sudo().message_post_with_view(
                    'vitalpet_promote.blog_post_template_new_post',
                    subject=post.name,
                    values={'post': post},
                    subtype_id=self.env['ir.model.data'].sudo().xmlid_to_res_id('vitalpet_promote.mt_blog_blog_published'))
            return True
        return False
    ...
    def get_access_action(self):
        ...
        return {
            ...
            'url': '/promote/%s/post/%s' % (self.blog_id.id, self.id),
            'target': 'self',
            'target_type': 'public',
            'res_id': self.id,
        }
...
class PromotePostWizard(models.TransientModel):

    _name = 'promote.post.wizard'
    
    name = fields.Char('Title', translate=True, default='')
    service = fields.Char('Service', translate=True, default='')
    sequence = fields.Integer('Sequence')
    short_text = fields.Char('Short Description', translate=True)
    blog_id = fields.Many2one('promote.blog', 'Blog', required=True, ondelete='cascade')
    tag_ids = fields.Many2many('promote.tag', string='Tags')
    company_id = fields.Many2one('res.company', required=True, string='Practice')
    image = fields.Binary("Image", required=True)
    
    #@api.onchange('blog_id')
    #def on_change_blog_id(self):
    #    categs = self.env['promote.blog'].search(
    #        [('name', 'in', ['Services', 'News', 'Promotions'])])
    #    return {'domain':{'blog_id':[('id','in',categs.ids)]}}

    @api.multi
    def action_create_post(self):
        new_blog_post = self.env['promote.post'].create({
