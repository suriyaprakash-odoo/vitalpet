...
class Meeting(models.Model):
    ...
    def validate(self, meeting):
        ...
        user_id = meeting.user_id and meeting.user_id.id or self._uid
        emp_id = self.env['hr.employee'].search([('user_id', '=', user_id)],
                                                limit=1)
        if not emp_id or not emp_id.product_id or not emp_id.journal_id:
        ...
        vals_line['account_id'] =\
            project_id.analytic_account_id and project_id.\
...
class ProjectScrumSprint(models.Model):
    ...
    def _get_velocity(self):
        ...
        for sprint in self:
            velocity = 0
            for backlog_id in self.env['project.scrum.product.backlog'].search(
                    [('sprint_id', '=', sprint.id)]):
    ...
    def _compute_meeting_count(self):
        ...
        for sprint_id in self:
            sprint_id.meeting_count = self.env['project.scrum.meeting'].search(
                [('sprint_id', '=', sprint_id.id)], count=True)
...
class ProjectTask(models.Model):
    ...
    incidents = fields.One2many('project.issue', 'task_id', 'Incidents')
...
class ProjectProject(models.Model):
    ...
    alias_model = fields.Selection(
        ...
        required=False,
        default=False,
        help="The kind of document created"
        "when an email is received on this project's email alias"
    )
    ...
    def create(self, vals):
        if not vals.get('alias_name'):
            raise ValidationError(_(
                """One of the following configuration is still missing.\n
                * Please configure Alias Domain in General Settings.
                (Setting -> General Seetings)
                * Please define Email alias
                """))
...
class ProjectIssue(models.Model):
    ...
    name = fields.Char('Incident', size=256, translate=True)
    description = fields.Html('Description')
    product_backlog_id = fields.Many2one(
        'project.scrum.product.backlog',
        related='task_id.product_backlog_id',
        string='Request',
        readonly=True,
        store=True
    )
    ...
    issue_number = fields.Char(
        ...
        readonly=True,
        copy=False,
        help="Sequence of Incidence Number"
    )
    ...
    def _track_subtype(self, init_values):
        ...
        if 'kanban_state' in init_values and self.kanban_state == 'blocked':
            ...
            return 'project_scrum_agile.mt_issue_started'
        elif 'user_id' in init_values and self.user_id:  # assigned -> new
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        ...
        if name:
            ...
            if operator in positive_operators:
                issue = self.search(
                    [('issue_number', operator, name)] + args, limit=limit)
                if not issue:
                    issue = self.search(args +
                                        [('issue_number', operator, name)],
                                        limit=limit)
                    if not limit or len(issue) < limit:
                        limit2 = (limit - len(issue)) if limit else False
                        issue += self.search(args + [('name', operator, name)],
                                             limit=limit2)
                if not issue:
                    ptrn = re.compile('(\[(.*?)\])')
                    res = ptrn.search(name)
                    if res:
                        issue = self.search(
                            [('issue_number', '=', res.group(2))] + args,
                            limit=limit)
        else:
            issue = self.search(args, limit=limit)
        return issue.name_get()

    @api.model
    def create(self, vals):
        if ('issue_number' not in vals) or (vals['issue_number'] is False):
            vals['issue_number'] = self.env['ir.sequence'].\
                next_by_code('project.issue.number') or '/'
        res = super(ProjectIssue, self).create(vals)
        return res

    @api.multi
    def button_create_task(self):
        """ Create Task From Project Issue
        @param self: The object pointer
        """
        context = self._context
        if context is None:
        ...
        if not stage_id:
            raise ValidationError(_(
                'cancel Stage Not Found! Please Create one'))
        for issue in self:
            issue.write({'stage_id': stage_id.id})
            data = {}
            data.update({
                'name': issue.name,
                'user_id': issue.user_id and issue.user_id.id or False,
                'project_id':
                    issue.project_id and issue.project_id.id or False,
                'email': issue.email_from
             })
            if issue.partner_id:
                data.update({'partner_id': issue.partner_id and issue.
                             partner_id.id or False})
            data.update({'description': issue.description})
            message_follower_ids = issue.message_follower_ids or []
            for partner_ids in message_follower_ids:
                message_follower_ids =\
                    self.env['mail.followers'].\
                    _add_follower_command(self._name,
                                          [issue.id],
                                          {partner_ids.partner_id.id: None},
                                          {},
                                          force=False)[0]
            data.update({'message_follower_ids': message_follower_ids})
            task_id = self.env['project.task'].create(data)
            if task_id:
                for record in issue.message_ids:
                    task_id.message_post(body=record.body)
            issue.write({'task_id': task_id.id})
        return True
